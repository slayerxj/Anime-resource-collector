...
var formatAliases = library.getFormatAliases().map(function (cur) {
  return cur.toUpperCase();
});
var workNames = library.getWorkNames().map(function (cur) {
  return cur.toUpperCase();
});
var subtitleProviders = library.getSubtitleProviders().map(function (cur) {
  return cur.toUpperCase();
});
var completeAliases = library.getCompleteAliases().map(function (cur) {
  return cur.toUpperCase();
});
...
var rank = function (item, lines) {
  ...
  if (
    subtitleProviders.some(function (cur) {
  ...
  if (
    completeAliases.some(function (cur) {
      return upperItemName.indexOf(cur) !== -1;
    })
  ) {
